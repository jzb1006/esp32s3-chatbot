...
class DeviceSimulatorTest(unittest.TestCase):
    ...
    def test_stream_chat_request_includes_device_token_and_conversation(self):
        calls = []

        def fake_post_stream(url, headers, payload):
            calls.append((url, headers, payload))
            return [
        ...
        events = list(client.stream_chat("你好"))

        self.assertEqual(client.conversation_id, "conv-1")
        self.assertEqual(events[1]["data"]["delta"], "你")
        url, headers, payload = calls[0]
        self.assertEqual(url, "http://127.0.0.1:8766/api/chat/stream")
        self.assertEqual(headers["X-Device-Token"], "device-secret")
        self.assertEqual(payload["device_id"], "esp32-dev-001")
        self.assertEqual(payload["prompt"], "你好")
