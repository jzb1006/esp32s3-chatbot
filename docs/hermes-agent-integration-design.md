...
| 版本 | 日期 | 变更摘要 |
|------|------|---------|
| v1 | 2026-06-14 | 初版：评估把 Nous Research **Hermes Agent** 作为 `llm_admin` 后端的可行性；给出路径 A/B/C 对比、安全收紧清单、部署网络拓扑、超时与会话归属取舍、待拍板决策点 |
...
把 `llm_admin` 当前直连的"裸大模型"（DeepSeek `/chat/completions`）替换 / 升级为 **Hermes Agent** —— 一个带持久记忆、技能（skill）学习、工具调用、200+ 模型路由的自治 Agent。期望 ESP32 设备问出的问题，由 Hermes 这个"大脑"来回答，而不仅是一次性补全。

**关键利好（为什么这事很顺）：** `llm_admin/app_core.py` 的 `ChatService.chat()` 本就是标准 OpenAI 调用：
...
ESP32-S3 (esp32s3_wifi_provision.ino)
  ...
  ▼
llm_admin 后台 (server.py · 纯标准库 · 腾讯云 Docker · :8766)
...
| 鉴权 | `Authorization: Bearer <API_SERVER_KEY>` | 与 llm_admin 现有 `Bearer {api_key}` 完全兼容 |
...
| 流式 | `"stream":true` → SSE，含 `hermes.tool.progress` 事件 | llm_admin 当前 `stream:false`，无需改 |
...
```
ESP32-S3  ──/api/chat──▶  llm_admin (:8766)  ──/v1/chat/completions──▶  Hermes API server (:8642)
...
设备契约、固件、管理后台**全部不动**；llm_admin 仅改 3 个配置值（§13）。Hermes 落在 llm_admin 之后，对设备完全透明。
...
|---|---|---|---|
| llm_admin 改动 | 改 3 个配置值，**零代码** | 重写 `ChatService` 走 `/v1/responses` + 会话映射；`ConversationStore` 语义改为存 `response_id` | 基本绕过 llm_admin |
| 固件改动 | 无 | 无 | 需改（直连地址 / 鉴权 / 可能 SSE） |
| 会话/记忆归属 | **llm_admin 管**（Hermes 无状态） | **Hermes 管**（吃到记忆 / skill / 学习） | Hermes 管 |
...
**C 为何否决：** ① ESP32 要持有能驱动 terminal 的 Hermes 强 key，泄漏即宿主沦陷；② 丢掉 llm_admin 已有的 `device_token` 鉴权、限长、多设备会话、管理后台；③ 设备侧做 SSE / Bearer 更折腾。**llm_admin 作为"设备网关 + 鉴权 + 限流 + 后台"这层有独立价值，应保留，Hermes 放它后面。**
...
- **路径 A**：llm_admin 的 `ConversationStore` 继续按 `device_id+conversation_id` 存历史，每次把最近 N 轮塞进 `messages` 发给 Hermes；Hermes 当**无状态**补全。结果：Hermes 退化为"带工具的 LLM"，**它的跨会话记忆 / skill 自学习用不上**。
- **路径 B**：把"记历史"交给 Hermes（`/v1/responses` 的 `previous_response_id` 或命名 `conversation`）。llm_admin 只需维护 `device_id+conversation_id → Hermes 会话标识` 的映射，退化为纯网关。结果：吃到 Hermes 的记忆 / skill / 学习，但要处理 100 条 response 上限的 LRU 淘汰、会话映射持久化、改动面。

> 若最终选 B，建议补一条 ADR 记录"会话状态归属从 llm_admin 迁移到 Hermes"这一架构决策（满足"有明显取舍、未来可能被质疑"的 ADR 触发条件）。
...
2. **双层 system prompt**：llm_admin 的 `system_prompt` + `user_memory` 会随 messages 进入 Hermes，而 Hermes 自身也有人格 / 系统提示。两层可能叠加甚至打架。→ A 路径下建议**简化 llm_admin 的 system_prompt**，把人格交给 Hermes（`/personality`）。
...
2. **强 `API_SERVER_KEY`** —— 高熵随机，且**只在内网 / Docker 网络可达，绝不映射公网端口**（与 llm_admin 的 `8766` 不同，Hermes 的 `8642` 不应进腾讯云安全组放行列表）。
3. **保留 `device_token` 作第一道门** —— 设备侧鉴权不能因为后端换了 Hermes 而放松。
4. **最小权限部署** —— Hermes 容器 / 进程以非 root、受限文件系统、独立网络运行；即便工具被诱发，爆炸半径受限。

### 8.2 叠加在现有风险之上

`docs/deployment.md §9` 已记录 llm_admin 当前 **HTTP 明文**（Basic Auth base64、公网 8766）。Hermes 接入不改变这一点，但**放大了后果**：明文链路 + 后端能跑命令 = 风险等级上升。→ 建议把"上 HTTPS"从 deployment.md 的可选 TODO 提升为对接 Hermes 的前置项。

### 8.3 凭据语义变化（避免混淆）

切到 Hermes 后，llm_admin 配置里的 `api_key` 含义从"DeepSeek key（直接计费调模型）"变成"**Hermes 网关鉴权 key（`API_SERVER_KEY`）**"；真正调底层模型的 provider key（OpenRouter / DeepSeek / Nous Portal）改在 **Hermes 侧**配置。计费口径随之转移到 Hermes。

---

## 9. 部署与网络拓扑

现状：llm_admin 在腾讯云 CVM（`203.195.202.54`，4 核 / 3.6G，Ubuntu 24.04）Docker Compose 部署，端口 8766。Hermes 部署有两种位置：

| 方案 | 拓扑 | 优点 | 风险 / `[需实测]` |
|---|---|---|---|
| **同机同 Docker 网络**（推荐） | 同一 CVM 内新增 hermes 容器，与 llm_admin 同 compose 网络；llm_admin 用 `http://hermes:8642/v1` 互访 | 不暴露 8642 到公网；运维集中 | **3.6G 内存对 Hermes 是否够**（uv/node/ffmpeg + 若跑本地模型更紧）`[需实测]`；Hermes 默认仅听 127.0.0.1，需让它监听容器网络可达地址 `[需实测]` |
| **独立主机** | Hermes 单独一台，llm_admin 经内网 / 加密隧道访问 | 资源隔离；互不影响 | 多一台机器成本；内网打通 |
...
| A1-2 | llm_admin 后台改 3 配置值后，用管理页"测试聊天" | 返回 answer，且来自 Hermes |
| A1-3 | `device_simulator.py` 连续两轮提问 | 第二轮能延续上下文（llm_admin history 生效） |
...
llm_admin 管理页（`/admin/<secret>`）改三项，落到 `data/llm_config.json`：
...
冒烟 curl（搭好后先单测 Hermes，再切 llm_admin）：
...
> 切换前先 `cat /opt/llm_admin/data/llm_config.json` 备份原 `base_url`/`model`/`api_key`，便于回滚到 DeepSeek。

---

## 14. 语音交互链路全景（roadmap · 唤醒词 / 流式 / ASR / TTS 在哪做）

> 本项目目标是**单用户语音机器人**，硬件经语音与机主交互。本节定位语音链路各层的位置，供后续分阶段实施。`llm_admin` 现有 `/api/voice/chat`（返回 `voice_not_ready`）即这条链路的入口占位。

### 14.1 分层与位置

```
[ESP32-S3 设备本地]                                  [云端 / llm_admin + Hermes]
 I2S 麦克风
   │ 常驻低功耗监听
   ▼
...
| ④ LLM | 云端（llm_admin→Hermes）| 本设计主体 | Hermes（路径 B）|
...
- **llm_admin 侧**：当前 `stream:false` 写死、`/api/chat` 一次性 JSON。需新增 **SSE 透传**（如 `/api/chat/stream`）：向 Hermes 发 `stream:true`，边收边转发给设备 / TTS。
...
2. **阶段二（流式）**：llm_admin 加 SSE 透传，设备 / 模拟器支持读流，验证首字延迟改善。
